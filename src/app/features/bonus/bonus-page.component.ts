  ...
  async importFromCsvText(csvText?: string): Promise<void> {
    ...
    try {
      ...
      if (headerParts.length < 3) {
        this.csvImportResult = { type: 'error', message: 'ヘッダー行が不正です（3列必要：支給日,従業員,賞与額）' };
        return;
      }
      ...
      console.log('[bonus-page] CSV列インデックス:', { payDateIndex, employeeIndex, bonusAmountIndex });
      
      if (payDateIndex === -1 || employeeIndex === -1 || bonusAmountIndex === -1) {
        this.csvImportResult = { type: 'error', message: 'ヘッダーに「支給日」「従業員」「賞与額」の列が必要です' };
        return;
      }
      ...
      console.log('[bonus-page] 従業員リスト:', this.employees.map(emp => emp.name));
      ...
      for (const line of dataLines) {
        ...
        if (parts.length < headerParts.length) {
          errorCount++;
          errors.push(`行「${line}」: 列数が不足しています（期待: ${headerParts.length}列、実際: ${parts.length}列）`);
          continue;
        }
        ...
        if (!payDateStr || !payDateStr.match(/^\d{4}-\d{2}-\d{2}$/)) {
          errorCount++;
          errors.push(`行「${line}」: 支給日が不正です（YYYY-MM-DD形式が必要。実際の値: "${payDateStr}"）`);
          continue;
        }
        ...
        if (!column) {
          // 既存の列を支給日付で検索
          const existingColumn = this.bonusColumns.find(col => col.payDate === payDateStr);
          if (existingColumn) {
            ...
            if (column.id !== correctId) {
              ...
              console.log('[bonus-page] 既存の列のIDを更新:', { oldId, newId: correctId });
            }
        ...
        if (!employee) {
          errorCount++;
          const availableNames = this.employees.map(emp => emp.name).join(', ');
          errors.push(`行「${line}」: 従業員「${employeeName}」が見つかりません。利用可能な従業員: ${availableNames}`);
          continue;
        }
        ...
        console.log('[bonus-page] 賞与額文字列:', bonusAmountStr, '→ 数値:', bonusAmount);
        
        if (isNaN(bonusAmount) || bonusAmount < 0) {
          errorCount++;
          errors.push(`行「${line}」: 賞与額が不正です（実際の値: "${bonusAmountStr}"）`);
          continue;
        }
        ...
        console.log('[bonus-page] 賞与データを設定:', { key, amount: bonusAmount, employeeName: employee.name, payDate: payDateStr });
      ...
      console.log('[bonus-page] CSVインポート完了:', { successCount, errorCount, errors });
    } catch (error) {
