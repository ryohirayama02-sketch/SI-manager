...
export class SettingsPageComponent {
  ...
  async saveOffice(): Promise<void> {
    try {
      ...
      if (isPrefectureChanged && newPrefecture) {
        try {
          let updateCount = 0;
          if (officeNumber) {
            // officeNumberがある場合は、officeNumberで検索して更新
            updateCount =
            ...
            // officeNumberがnullの場合は、selectedOfficeIdで検索して更新
            updateCount =
              await this.employeeService.updateEmployeesPrefectureBySelectedOfficeId(
                this.selectedOffice.id,
                newPrefecture
              );
          }
