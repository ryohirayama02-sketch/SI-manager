...
class BonusServiceStub {
  ...
  async getBonusesForResult(employeeId: string, year: number): Promise<Bonus[]> {
    return Promise.resolve(this.bonusesForResult);
  }

  async getBonusesLast12Months(employeeId: string, payDate: string): Promise<Bonus[]> {
    return Promise.resolve(this.bonusesLast12Months);
  }
}
...
class MaternityLeaveServiceStub {
  ...
  isExemptForBonus(payDate: string, employee: Employee): { exempt: boolean; reason?: string } {
...
describe('BonusCalculationService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: EmployeeEligibilityService, useValue: employeeEligibilityServiceStub },
        { provide: SalaryCalculationService, useValue: salaryCalculationServiceStub },
  ...
  describe('1. 標準賞与額の丸め & 上限ロジック', () => {
    ...
    describe('1-2. applyBonusCaps', () => {
      ...
      it('健保・介保: 年度累計573万円上限（既に5,680,000円支給済）', async () => {
        bonusServiceStub.bonusesForResult = [
          { amount: 5680000, employeeId: 'emp1', year: 2025, month: 1, payDate: '2025-01-15' } as unknown as Bonus,
        ];
  ...
  describe('2. 退職月 & 月末在籍ロジック', () => {
    ...
    describe('2-2. calculateBonus 経由（退職月で月末在籍なし）', () => {
      it('退職月で月末在籍がない場合、全保険料が0円になる', async () => {
        ...
        const result = await service.calculateBonus(employee, 'emp1', 1000000, '2025-04-20', rates);
  ...
  describe('3. 産休・育休による免除', () => {
    describe('3-1. 産休中の賞与免除', () => {
      it('産休期間中は全保険料が0円になる', async () => {
        ...
        const result = await service.calculateBonus(employee, 'emp1', 1000000, '2025-07-15', rates);
        expect(result).not.toBeNull();
        if (!result) return;
        ...
        expect(result.pensionEmployee).toBe(0);
        expect(result.reasons).toContain(
          jasmine.stringContaining('産前産後休業中のため、賞与保険料は免除されます')
        );
      });
    });
    ...
    describe('3-2. 育休中（3条件満たす）で免除', () => {
      it('育休期間中で3条件を満たす場合、全保険料が0円になる', async () => {
        ...
        const result = await service.calculateBonus(employee, 'emp1', 1000000, '2025-10-15', rates);
        expect(result).not.toBeNull();
        if (!result) return;
  ...
  describe('4. 年齢到達による停止（AgeFlags 経由）', () => {
    describe('4-1. 70歳以上 → 厚生年金0円', () => {
      it('70歳以上では厚生年金保険料が0円になる', async () => {
        ...
        const result = await service.calculateBonus(employee, 'emp1', 1000000, '2025-05-15', rates);
        expect(result).not.toBeNull();
        if (!result) return;
    ...
    describe('4-2. 75歳以上 → 健保・介保0円', () => {
      it('75歳以上では健保・介保・厚年すべて0円になる', async () => {
        ...
        expect(result.pensionEmployee).toBe(0);
        expect(result.reasons).toContain(
          jasmine.stringContaining('75歳到達月のため健保・介保の賞与保険料は停止されます')
        );
      });
    });
  });
  ...
  describe('5. 賞与 → 給与扱い（3回ルール）', () => {
    describe('5-1. 過去12ヶ月で3回以内 → 賞与扱い', () => {
      it('3回以内の場合は賞与扱い', async () => {
        ...
        bonusServiceStub.bonusesLast12Months = [
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 1, payDate: '2025-01-15' } as unknown as Bonus,
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 4, payDate: '2025-04-15' } as unknown as Bonus,
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 6, payDate: '2025-06-15' } as unknown as Bonus,
        ];
        bonusServiceStub.bonusesForResult = [];
        employeeEligibilityServiceStub.ageFlags = {
          isNoHealth: false,
        ...
        expect(result.salaryInsteadReasons).toContain(
          jasmine.stringContaining('過去12ヶ月の賞与支給回数が3回（3回以内）のため賞与扱い')
        );
        expect(salaryCalculationServiceStub.addBonusAsSalary).not.toHaveBeenCalled();
      });
    });
    ...
    describe('5-2. 過去12ヶ月で4回以上 → 給与扱い', () => {
      it('4回以上の場合は給与扱い', async () => {
        ...
        bonusServiceStub.bonusesLast12Months = [
          ...
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 4, payDate: '2025-04-15' } as unknown as Bonus,
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 6, payDate: '2025-06-15' } as unknown as Bonus,
          { amount: 1000000, employeeId: 'emp1', year: 2025, month: 7, payDate: '2025-07-15' } as unknown as Bonus,
        ];
        bonusServiceStub.bonusesForResult = [];
        employeeEligibilityServiceStub.ageFlags = {
          isNoHealth: false,
        ...
        const result = await service.calculateBonus(employee, 'emp1', 1000000, '2025-08-15', rates);
        expect(result).not.toBeNull();
        if (!result) return;
        ...
        expect(result.healthEmployee).toBe(0);
        expect(result.pensionEmployee).toBe(0);
        expect(salaryCalculationServiceStub.addBonusAsSalary).toHaveBeenCalledWith(
          'emp1',
          2025,
          8,
          1000000
  ...
  describe('6. calculatePremiums 単体の挙動', () => {
    it('ageFlags.isNoHealth=false, isNoPension=false, isCare2=true のケース', () => {
      ...
      const result = service.calculatePremiums(1000000, 1000000, 40, ageFlags, rates);

      expect(result.healthEmployee).toBe(Math.floor(1000000 * rates.health_employee));
      expect(result.careEmployee).toBe(Math.floor(1000000 * rates.care_employee));
      expect(result.pensionEmployee).toBe(Math.floor(1000000 * rates.pension_employee));
    });
    ...
    it('isNoHealth=true → health/care が 0 になる', () => {
      ...
      const result = service.calculatePremiums(1000000, 1000000, 75, ageFlags, rates);
    ...
    it('isNoPension=true → pension が 0 になる', () => {
      ...
      const result = service.calculatePremiums(1000000, 1000000, 70, ageFlags, rates);
  ...
  describe('7. buildReasons / determineReportRequirement / checkReportRequired', () => {
    describe('7-1. buildReasons', () => {
      it('各フラグを true にしたとき、それぞれの文言が reasons に含まれること', () => {
        ...
        expect(reasons4).toContain(
          jasmine.stringContaining('70歳到達月のため厚生年金の賞与保険料は停止されます')
        );
        ...
        expect(reasons6).toContain(
          jasmine.stringContaining('過去1年間の賞与支給回数が3回を超えているため')
        );
        ...
        expect(reasons7).toContain(
          jasmine.stringContaining('健保・介保の年度上限（573万円）を適用しました')
        );
        ...
        expect(reasons8).toContain(
          jasmine.stringContaining('厚生年金の1回あたり上限（150万円）を適用しました')
        );
      });
    });
    ...
    describe('7-2. determineReportRequirement', () => {
      it('退職月月末在籍なし → requireReport=false', () => {
        ...
        expect(result.requireReport).toBe(false);
        expect(result.reportReason).toContain('退職月の月末在籍が無いため賞与支払届は不要');
      });
      ...
      it('産休/育休免除 → requireReport=false', () => {
        ...
        expect(result.requireReport).toBe(false);
        expect(result.reportReason).toContain('産前産後休業中の賞与は免除対象のため賞与支払届は不要です');
      });
      ...
      it('75歳以上 → requireReport=false', () => {
        ...
        expect(result.requireReport).toBe(false);
        expect(result.reportReason).toContain('75歳到達月で健康保険・介護保険の資格喪失のため賞与支払届は不要です');
      });
      ...
      it('給与扱い → requireReport=false', () => {
        ...
        expect(result.requireReport).toBe(false);
        expect(result.reportReason).toContain('年度内4回目以降の賞与は給与扱いとなるため賞与支払届は不要です');
      });
      ...
      it('通常ケース → requireReport=true かつ reportDeadline が payDate+5日', () => {
        ...
        expect(result.reportReason).toContain('支給された賞与は社会保険の対象となるため、賞与支払届が必要です');
        const expectedDeadline = new Date(payDate);
        expectedDeadline.setDate(expectedDeadline.getDate() + 5);
        expect(result.reportDeadline).toBe(expectedDeadline.toISOString().split('T')[0]);
      });
    });
  ...
  describe('8. エラーチェック（checkErrors）', () => {
    it('支給日が入社前の場合', () => {
      ...
      expect(result.errorMessages).toContain('支給日が在籍期間外です（入社前）');
    });
    ...
    it('支給日が退職後の場合', () => {
      ...
      expect(result.errorMessages).toContain('支給日が在籍期間外です（退職後）');
    });
    ...
    it('70歳以上なのに isOverAge70=false かつ pensionEmployee > 0 のケース', () => {
      ...
      expect(result.errorMessages).toContain('70歳以上は厚生年金保険料は発生しません');
    });
    ...
    it('75歳以上なのに isOverAge75=false かつ health/care > 0 のケース', () => {
      ...
      expect(result.errorMessages).toContain('75歳以上は健康保険・介護保険は発生しません');
    });
    ...
    it('賞与回数ロジックの矛盾', () => {
      ...
      expect(result.errorMessages).toContain('賞与の支給回数ロジックに矛盾があります');
    });
  });
});
