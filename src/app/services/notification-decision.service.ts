...
export class NotificationDecisionService {
  ...
  checkTeijiNotification(
    ...
    if (
      teijiResult.grade === 0 ||
      teijiResult.standardMonthlyRemuneration === 0
    ) {
      reasons.push(
        '定時決定により等級が決定されていないため、算定基礎届は不要'
      );
      return {
        type: 'teiji',
        required: false,
    ...
    if (currentGrade === 0) {
      reasons.push(
        '現行等級が設定されていないため、算定基礎届の提出要否を判定できません'
      );
      return {
        type: 'teiji',
        required: false,
    ...
    if (gradeDiff >= 2) {
      reasons.push(
        `定時決定により等級${currentGrade}から等級${teijiResult.grade}に変更（${gradeDiff}等級差）のため、算定基礎届の提出が必要`
      );
      // 提出期限：7月10日
      ...
      reasons.push(
        `定時決定により等級${currentGrade}から等級${teijiResult.grade}に変更（${gradeDiff}等級差）のため、算定基礎届は不要`
      );
      return {
        type: 'teiji',
        required: false,
  ...
  checkSuijiNotification(
    ...
    if (suijiResult.diff >= 2) {
      ...
      reasons.push(
        `固定的賃金変動により等級${suijiResult.currentGrade}から等級${suijiResult.newGrade}に変更（${suijiResult.diff}等級差）のため、月額変更届は不要`
      );
      return {
        type: 'suiji',
        required: false,
