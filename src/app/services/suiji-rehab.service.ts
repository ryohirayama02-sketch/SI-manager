...
export class SuijiRehabService {
  ...
  checkRehabSuiji(
    ...
    // 復職情報がない場合はスキップ
    if (!returnMonth || !returnYear || isNaN(returnMonth) || isNaN(returnYear)) return [];

    // 復職年が現在の年と異なる場合はスキップ
    const yearNum = parseInt(year, 10);
    if (isNaN(yearNum) || yearNum !== returnYear) return [];
  ...
  getRehabHighlightMonths(employee: Employee, year: string): number[] {
    ...
    // 復職年が現在の年と異なる場合は空配列
    const yearNum = parseInt(year, 10);
    if (isNaN(yearNum) || isNaN(returnYear) || yearNum !== returnYear) return [];
